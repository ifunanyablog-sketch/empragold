      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="relative mb-10">
          ...
          <input
            ...
            className="w-full bg-[#1A1A1D] border border-white/10 focus:border-[#C9A227] rounded-2xl pl-11 pr-4 py-3.5 text-sm text-white focus:outline-none transition-all"
          />
        </div>
        ...
        <div className="space-y-10">
          {faqData.map((cat, catIdx) => (
            <div key={catIdx} className="space-y-4">
              <h3 className="text-xl font-serif font-bold text-[#C9A227] border-b border-white/10 pb-2">
                {cat.category}
              </h3>
              ...
              <div className="space-y-3">
                {cat.items.map((item, itemIdx) => {
                  ...
                  return (
                    <div
                      key={itemIdx}
                      className="bg-[#1A1A1D] border border-white/10 rounded-2xl overflow-hidden transition-all"
                    >
                      <button
                        onClick={() => toggleAccordion(globalIdx)}
                        className="w-full p-5 text-left font-serif font-semibold text-white text-base flex justify-between items-center gap-4 hover:text-[#C9A227] transition-colors"
                      >
                        <span>{item.q}</span>
        ...
        {/* Support Callout */}
        <div className="mt-16 p-8 bg-[#1A1A1D] border border-[#C9A227]/30 rounded-2xl text-center space-y-4">
          <HelpCircle className="w-10 h-10 text-[#C9A227] mx-auto" />
          <h3 className="text-xl font-serif font-bold text-white">Have a Specific Legal or Market Question?</h3>
          <p className="text-xs text-gray-400 max-w-md mx-auto">
            Our Private Client Advisors are available 24/7 to provide confidential guidance.
          </p>
          <Link
            to="/contact"
            className="inline-block bg-[#C9A227] text-black font-semibold text-xs uppercase tracking-wider px-6 py-3 rounded-xl hover:bg-[#D4AF37] transition-all"
          >
